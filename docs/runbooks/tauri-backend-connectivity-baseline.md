...
|---|---|---|---|
| Client transport entry points | `apps/desktop-tauri/src/api/client.ts`; `libs/ts/@gb/api-client/src/index.ts` | Desktop client creates `GbApiClient` and injects auth headers except for `/health` paths. Shared client uses `requestJson()` for all HTTP calls, has reconnecting topic WS support, and appends `last_seq` query parameter during reconnect attempts. | `createDesktopApiClient` + `shouldAttachAuthHeader`/`resolveAuthorizationHeader`; `GbApiClient.requestJson`; `connectTopicWebSocket`; `withResumeSeq`. |
| Auth behavior | `apps/api-control-plane/app/core/auth.py`; `apps/desktop-tauri/src/settings/tokenStorage.ts`; `apps/desktop-tauri/src-tauri/src/main.rs` | Backend middleware validates one static bearer token (`GB_API_AUTH_TOKEN`), sets request scope, and bypasses configured health paths. Desktop token is persisted in OS keychain via Tauri commands with in-memory fallback for non-Tauri contexts. No token rotation or expiry lifecycle logic is present in client or backend auth middleware. | `BearerTokenAuthMiddleware.dispatch`; config `env_prefix="GB_"` + `api_auth_token`; token storage `save_api_token`/`get_api_token`; TS token storage wrappers. |
